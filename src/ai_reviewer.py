...
load_dotenv(PROJECT_ROOT / "ui" / ".env")
...
if groq_key:
    # Groq — free, OpenAI-compatible, high rate limits on llama3-8b
    client = OpenAI(
        api_key=groq_key,
...
elif openai_key and openai_key != "REPLACE_WITH_A_NEW_OPENAI_API_KEY":
...
def review_code_with_ai(code: str) -> str:
    ...
    if client is None:
        raise RuntimeError(
            "No API key configured. Set GROQ_API_KEY, GEMINI_API_KEY, or OPENAI_API_KEY in your .env file."
        )
